...
import (
	...
	frameworkphrases "github.com/ali-mahdavi-dev/shikposh-framework/errors/phrases"
)
...
func (h *ReviewCommandHandler) UpdateReviewHelpfulHandler(ctx context.Context, cmd *commands.UpdateReviewHelpful) error {
	err := h.uow.Do(ctx, func(ctx context.Context) error {
		// Validate enum type
		if !cmd.Type.IsValid() {
			return apperrors.Validation(frameworkphrases.DefaultValidationID, fmt.Sprintf("invalid review helpful type: %s, must be 'helpful' or 'notHelpful'", cmd.Type))
		}

		review, err := h.uow.Review(ctx).FindByID(ctx, cmd.ReviewID)
		if err != nil {
			if errors.Is(err, repository.ErrReviewNotFound) {
				return apperrors.NotFound(phrases.ReviewNotFound)
			}
